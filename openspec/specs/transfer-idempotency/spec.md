...
Make `POST /api/v1/transfers` safely retryable by clients. When a client passes an `Idempotency-Key` request header, the service runs the [[fund-transfer]] pipeline exactly once for that key and replays the identical stored response on subsequent retries, returning the canonical [[api-error-contract]] envelope unchanged (including the original `timestamp` field). Keys reused with a different request body produce a 422 `IDEMPOTENCY_KEY_REUSED`; in-flight collisions produce a 409 `CONCURRENT_IDEMPOTENT_REQUEST`. The contract for the header is published through [[contract-first-api]], and replays bypass the [[metrics-exposure]] transfer instrumentation so retries do not skew counters.
