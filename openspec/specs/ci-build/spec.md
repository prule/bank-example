...
Provide an automated GitHub Actions workflow that compiles the multi-module Gradle project and runs its test suite on every push and pull request, so regressions are caught before merge. Owns the contract for CI triggers, toolchain, caching, and failure reporting — not deployment, release, coverage, or static analysis.
