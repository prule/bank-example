...
- **THEN** the hand-written `OpenApiController` (or equivalent) implements the generated interface from `com.bank.core.api`, with method signatures, parameter annotations, and return types matching the generated interface
...
- **WHEN** the codebase is inspected after this change
- **THEN** generated interfaces live in `com.bank.core.api`, generated DTOs live in `com.bank.core.dto`, and ArchUnit confirms no class under `com.bank.core.domain` or `com.bank.core.application` imports from either generated package (see [[project-setup]])
...
- **THEN** the F00 ArchUnit boundary test fails the build
...
The error envelope schema SHALL be defined exactly once in the OpenAPI document, under `components.schemas.ErrorEnvelope`, and every `4xx`/`5xx` response declared anywhere in the document SHALL reference that schema via `$ref`. F03 owns the code taxonomy that populates the envelope; F04 owns the schema's location and the referencing pattern.

#### Scenario: One error schema referenced everywhere

- **WHEN** the OpenAPI document is inspected
- **THEN** the error envelope is defined exactly once under `components.schemas.ErrorEnvelope` (sourced from `schemas/error-envelope.yaml`), and every `4xx`/`5xx` response across every path declared in this change references it via `$ref: '#/components/schemas/ErrorEnvelope'`
...
- **THEN** the change is rejected at review (no automated check at this layer; the requirement is normative and reinforced by F03 when codes are introduced)
