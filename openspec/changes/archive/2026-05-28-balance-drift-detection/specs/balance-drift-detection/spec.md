...
- **THEN** the use case calls `movements.distinctAccountIdsInWindow(...)` ZERO times, no `accounts.findById(...)` calls, no `accounts.save(...)` calls; the checkpoint advance via `checkpoints.save("balance_drift", ceiling)` IS still invoked (writing the same value); the returned `DriftReport` is `(floor, ceiling, inspected=0, drifted=0)`
...
- **WHEN** the clearing account's `account.balance` is manipulated out-of-band so it no longer matches the sum of its ledger movements, AND a transfer involving the clearing account commits so its id falls into the next audit window
...
- **WHEN** an audit tick examines an account whose `status` is already `SUSPENDED`
...
- **WHEN** the candidate set returned by `distinctAccountIdsInWindow(...)` contains an `AccountId` for which `accounts.findById(id)` returns `Optional.empty()`
...
The audit SHALL NOT block live transfers and live transfers SHALL NOT cause the audit to miss or double-count movements. The mechanism: (a) the audit's per-account sum is a read-only aggregate against an indexed column, taking no row locks on `ledger_movement`; (b) the audit's `accounts.save(...)` for a drifted account acquires only the row lock for that one account row; (c) the audit's `ceiling` is captured ONCE per tick from `MAX(id)`, and `MAX(id)` returns only committed rows, so a transfer that commits after the audit's `currentCeiling()` call has an id strictly greater than `ceiling` and is therefore deferred to the next tick by the `(floor, ceiling]` window predicate.

#### Scenario: Movements after ceiling roll into next tick
- **WHEN** an audit tick captures `ceiling = 100` and then, before the tick's per-account processing completes, a transfer commits two new `ledger_movement` rows with ids `101` and `102`
...
- **WHEN** a transfer commits while an audit tick is mid-flight processing a different account
- **THEN** the transfer's `account.balance` and `ledger_movement` writes commit without waiting for the audit's transaction
...
- **THEN** the captured INFO log contains exactly one line whose formatted message equals `balance drift tick: floor=<F>, ceiling=<C>, inspected=2, drifted=1`
...
- **WHEN** any tick completes successfully and produces a `DriftReport`
- **THEN** the returned `DriftReport` holds `inspected >= drifted` and is correctly populated with the window boundaries and counts
