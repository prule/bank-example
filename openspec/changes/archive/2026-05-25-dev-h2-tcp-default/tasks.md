...
- [x] 1.1 Modify `bootstrap/src/main/resources/application-dev.yaml`. Replace the existing `bank-core.h2.tcp-server` block with one whose `enabled` value is `true` (was `false`). Keep `port: 9092`. Replace the existing one-line comment with a multi-line block documenting:
...
- [x] 3.2 Create `bootstrap/src/test/java/com/bank/core/config/H2ServerInitializerDefaultProfileTest.java`:
...
- [x] 4.1 Run `./gradlew :bootstrap:test --tests "com.bank.core.config.*"`. Both new tests pass; existing config tests still pass.
- [x] 4.2 Run `./gradlew clean build`. All modules green; F00 ArchUnit suite still passes; no new Gradle dependency.
- [x] 4.3 Run `openspec change validate dev-h2-tcp-default --strict`. Confirm clean output.
- [x] 4.4 Run `openspec validate --specs`. Confirm all 12 capability specs validate.
